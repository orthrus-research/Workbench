...
class NativeForgeRegistriesTest {
    @TempDir Path temporary;

    @ParameterizedTest @ValueSource(strings = {"en", "tr", "source-edit"})
    void nativeRegistryContractsInFreshRestrictedJvm(String mode) throws Exception {
        ...
        var source = temporary.resolve("ForgeRegistryConformance.java");
        try (var input = getClass().getResourceAsStream("/ForgeRegistryConformance.java")) { Files.write(source, input.readAllBytes()); }
        ...
        var builder = new ProcessBuilder(Path.of(System.getProperty("java.home"), "bin/java").toString(), "-Xmx256m",
                ...
                "research.orthrus.axiom.ForgeRegistryConformance", registry, mode);
        builder.environment().clear();
        var process = builder.redirectOutput(output.toFile()).redirectError(error.toFile()).start();
        try {
            assertTrue(process.waitFor(45, java.util.concurrent.TimeUnit.SECONDS), "Forge registry probe timeout");
            assertEquals(0, process.exitValue(), Files.readString(error));
            var result = Json.object(Json.parse(Files.readString(output)));
            if (mode.equals("source-edit")) assertEquals(0, ((Number) result.get("firstId")).intValue());
            else {
                assertEquals(8192, ((Number) result.get("operations")).intValue());
                assertEquals(11, ((Number) result.get("scenarios")).intValue());
