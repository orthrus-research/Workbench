...
import static research.orthrus.axiom.nativeconstruction.MaterialVoltages.M;
...
public class OreDictUnifier {
    ...
    public static Comparator<ItemAndMetadata> getSimpleItemStackComparator() {
        if (stackComparator == null) {
            List<String> modPriorities = Arrays.asList(FluidEnvironment.current().modPriorities());
    ...
    public static void registerOre(ItemStack itemStack, OrePrefix orePrefix,  FluidMaterial material) {
        registerOre(itemStack, orePrefix.name(), material);
    }

    public static void registerOre(ItemStack itemStack, String customOrePrefix,  FluidMaterial material) {
        if (itemStack.func_190926_b()) return;
    ...
    public static void onItemRegistration(OreRegisterEvent event) {
        ...
        ItemVariantMap.Mutable<Set<String>> entry = stackOreDictName.computeIfAbsent(event.getOre().func_77973_b(),
                item -> item.func_77614_k() ? new MultiItemVariantMap<>() : new SingleItemVariantMap<>());
        ...
        List<ItemStack> itemStackListForOreDictName = oreDictNameStacks.computeIfAbsent(oreName,
                k -> new ArrayList<>());
        addAndSort(itemStackListForOreDictName, event.getOre().func_77946_l(), getItemStackComparator());

        // and try to transform registration name into OrePrefix + FluidMaterial pair
        ...
        if (orePrefix == null) {
            ...
            for (MaterialRegistry registry : FluidEnvironment.current().runtime().materials().getRegistries()) {
                ...
                for (int i = 0; i < splits.size(); i++) {
                    ...
                    String underscoreName = FluidSupport.toLowerCaseUnderscore(possibleMaterialName); // basaltic_mineral_sand
                    FluidMaterial possibleMaterial = (FluidMaterial) registry.getObject(underscoreName); // Materials.BasalticSand
                    if (possibleMaterial == null) {
                        // if we didn't find real material, try using marker material registry
                        possibleMaterial = FluidEnvironment.current().markers().getMarkerMaterial(underscoreName);
                    }
    ...
    public static Set<String> getOreDictionaryNames( ItemStack itemStack) {
        if (itemStack.func_190926_b()) return Collections.emptySet();
        ItemVariantMap<Set<String>> nameEntry = stackOreDictName.get(itemStack.func_77973_b());
        ...
        Set<String> wildcardNames = itemDamage == ItemConstants.W ? null : nameEntry.get(ItemConstants.W);
        if (names == null) {
    ...
    public static ItemVariantMap<Set<String>> getOreDictionaryEntry( Item item) {
    ...
    public static ItemVariantMap<Set<String>> getOreDictionaryEntryOrEmpty( Item item) {
        ItemVariantMap.Mutable<Set<String>> entry = stackOreDictName.get(item);
        return entry == null ? ItemVariantMap.empty() : ItemVariantMap.unmodifiableSetView(entry);
    }

    public static boolean hasOreDictionaryEntry( Item item) {
        return stackOreDictName.containsKey(item);
    }

    public static boolean hasOreDictionary( ItemStack itemStack,  String oreDictName) {
        ...
        if (itemDamage == ItemConstants.W) return false;

        Set<String> wildcardNames = nameEntry.get(ItemConstants.W);
    ...
    public static ItemStack getUnificated(ItemStack itemStack) {
        if (itemStack.func_190926_b()) return ItemStack.field_190927_a;
    ...
    public static ItemStack get(OrePrefix orePrefix, FluidMaterial material) {
        return get(orePrefix, material, 1);
    }

    public static ItemStack get(OrePrefix orePrefix, FluidMaterial material, int stackSize) {
        UnificationEntry unificationEntry = new UnificationEntry(orePrefix, material);
    ...
    public static ItemStack get(String oreDictName) {
        ...
        if (itemStacks == null || itemStacks.size() == 0) return ItemStack.field_190927_a;
    ...
    public static ItemStack getDust(FluidMaterial material, long materialAmount) {
    ...
    public static ItemStack getIngot(FluidMaterial material, long materialAmount) {
     ...
     * Returns ItemStack.field_190927_a if neither exist.
     */
    public static ItemStack getIngotOrDust(FluidMaterial material, long materialAmount) {
        ItemStack ingotStack = getIngot(material, materialAmount);
        if (ingotStack != ItemStack.field_190927_a) return ingotStack;
    ...
    private static <T> T getOrWildcard( Map<ItemAndMetadata, T> map,
                                        ItemAndMetadata key) {
        T t = map.get(key);
        if (t != null) return t;
