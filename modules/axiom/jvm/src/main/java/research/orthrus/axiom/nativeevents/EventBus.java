// Relocated selected Cleanroom source; see spec/native-events.md.
/*
 * Minecraft Forge
...
public class EventBus implements IEventExceptionHandler
{
    ...
    private Map<Object,EventContext.Owner> listenerOwners = new MapMaker().weakKeys().weakValues().makeMap();
    ...
    public void register(Object target)
    {
        ...
        EventContext.Owner activeModContainer = EventContext.instance().activeModContainer();
        if (activeModContainer == null)
        {
            EventContext.log.error("Unable to determine registrant mod for {}. This is a critical error and should be impossible", target, new Throwable());
            activeModContainer = EventContext.instance().getMinecraftModContainer();
        }
    ...
    private void register(Class<?> eventType, Object target, Method method, final EventContext.Owner owner)
    {
        try
        {
        ...
        {
            EventContext.log.error("Error registering event handler: {} {} {}", owner, eventType, method, e);
        }
    }
    ...
    {
        EventContext.log.warn("EventBus {} shutting down - future events will not be posted.", busID);
        shutdown = true;
    }
    ...
    {
        EventContext.log.error("Exception caught during firing event {}:", event, throwable);
        EventContext.log.error("Index: {} Listeners:", index);
        for (int x = 0; x < listeners.length; x++)
        {
            EventContext.log.error("{}: {}", x, listeners[x]);
        }
    }

    private record ContextSetterEventListener(
        EventContext.Owner owner,
        ASMEventHandler asm
    ) implements IEventListener {

        @Override
        public void invoke(Event e) {
            var loader = EventContext.instance();
