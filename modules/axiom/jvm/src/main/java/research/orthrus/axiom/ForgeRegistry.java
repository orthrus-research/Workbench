...
class ForgeRegistry<V extends IForgeRegistryEntry<V>> implements IForgeRegistryInternal<V>, IForgeRegistryModifiable<V>
{
    public static final boolean DEBUG = Boolean.parseBoolean(System.getProperty("forge.debugRegistryEntries", "false"));
    private final ForgeRegistryManager stage;
    private final BiMap<Integer, V> ids = HashBiMap.create();
    private final BiMap<NativeLocation, V> names = HashBiMap.create();
    private final Class<V> superType;
    private final Map<NativeLocation, NativeLocation> aliases = Maps.newHashMap();
    final Map<NativeLocation, ?> slaves = Maps.newHashMap();
    private final NativeLocation defaultKey;
    ...
    private final Set<NativeLocation> dummies = Sets.newHashSet();
    private final Set<Integer> blocked = Sets.newHashSet();
    private final Multimap<NativeLocation, V> overrides = ArrayListMultimap.create();
    ...
    ForgeRegistry(Class<V> superType, NativeLocation defaultKey, int min, int max,  CreateCallback<V> create,  AddCallback<V> add,  ClearCallback<V> clear,  ValidateCallback<V> validate, ForgeRegistryManager stage, boolean allowOverrides, boolean isModifiable,  DummyFactory<V> dummyFactory,  MissingFactory<V> missing)
    {
        this.stage = stage;
    ...
    @Override
    public V getValue(NativeLocation key)
    {
    ...
    @Override
    public NativeLocation getKey(V value)
    {
        NativeLocation ret = this.names.inverse().get(value);
    ...
    @Override
    public Set<Entry<NativeLocation, V>> getEntries()
    {
    ...
    @Override
    public <T> T getSlaveMap(NativeLocation name, Class<T> type)
    {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public void setSlaveMap(NativeLocation name, Object obj)
    {
        ((Map<NativeLocation, Object>)this.slaves).put(name, obj);
    }
    ...
    public int getID(NativeLocation name)
    {
    ...
    private int getIDRaw(NativeLocation name)
    {
    ...
    ForgeRegistry<V> copy(ForgeRegistryManager stage)
    {
    ...
    int add(int id, V value)
    {
        ...
        String owner = mc == null || mc.injectedFmlContainer() ? null : mc.getModId().toLowerCase();
        return add(id, value, owner);
    }

    int add(int id, V value, String owner)
    {
        NativeLocation key = value == null ? null : value.getRegistryName();
        ...
        {
            ForgeRegistryLog.bigWarning("Registry {}: The object {} has been registered twice for the same name {}.", this.superType.getSimpleName(), value, key);
        ...
        if (oldEntry != null) // duplicate name
        {
            ...
            if (DEBUG)
                ForgeRegistryLog.log.debug("Registry {} Override: {} {} -> {}", this.superType.getSimpleName(), key, oldEntry, value);
        ...
        if (isDelegated)
        {
            ...
            if (oldEntry != null)
            {
                ...
                if (this.stage == ForgeRegistryManager.ACTIVE)
        ...
        if (this.dummies.remove(key) && DEBUG)
            ForgeRegistryLog.log.debug("Registry {} Dummy Remove: {}", this.superType.getSimpleName(), key);

        if (DEBUG)
            ForgeRegistryLog.log.trace("Registry {} add: {} {} {} (req. id {})", this.superType.getSimpleName(), key, idToUse, value, id);

        return idToUse;
    }

    private V getRaw(NativeLocation key)
    {
        V ret = this.names.get(key);
        key = this.aliases.get(key);
        while (ret == null && key != null)
    ...
    void addAlias(NativeLocation from, NativeLocation to)
    {
        if (this.isLocked())
            throw new IllegalStateException(String.format("Attempted to register the alias %s -> %s to late", from, to));
        if (from.equals(to))
        {
            ForgeRegistryLog.log.warn("Registry {} Ignoring invalid alias: {} -> {}", this.superType.getSimpleName(), from, to);
            return;
        }
        this.aliases.put(from, to);
        if (DEBUG)
            ForgeRegistryLog.log.trace("Registry {} alias: {} -> {}", this.superType.getSimpleName(), from, to);
    }

    void addDummy(NativeLocation key)
    {
        if (this.isLocked())
            throw new IllegalStateException(String.format("Attempted to register the dummy %s to late", key));
        this.dummies.add(key);
        if (DEBUG)
            ForgeRegistryLog.log.trace("Registry {} dummy: {}", this.superType.getSimpleName(), key);
    }
    ...
    boolean isDummied(NativeLocation key)
    {
        return this.dummies.contains(key);
    }


    void validateContent(NativeLocation registryName)
    {
        try
        {
        ...
        {
            int id = getID(obj);
            NativeLocation name = getKey(obj);
    ...
    void sync(NativeLocation name, ForgeRegistry<V> from)
    {
        if (DEBUG)
            ForgeRegistryLog.log.debug("Registry {} Sync: {} -> {}", this.superType.getSimpleName(), this.stage.getName(), from.stage.getName());
        if (this == from)
        ...
        */
        ...
        for (Entry<NativeLocation, V> entry : from.names.entrySet())
        {
            ...
            if (overrides.isEmpty())
            {
                ...
                if (id != realId && id != -1)
                {
                    ForgeRegistryLog.log.warn("Registry {}: Object did not get ID it asked for. Name: {} Expected: {} Got: {}", this.superType.getSimpleName(), entry.getKey(), id, realId);
                    errored = true;
                }
            }
            else
            {
                ...
                for (V value : overrides)
                {
                    ...
                    if (owner == null)
                    {
                        ForgeRegistryLog.log.warn("Registry {}: Override did not have an associated owner object. Name: {} Value: {}", this.superType.getSimpleName(), entry.getKey(), value);
                        errored = true;
                        continue;
                    }
    ...
    @Override
    public V remove(NativeLocation key)
    {
        if (!this.isModifiable)
        ...
        if (value != null)
        {
            ...
            if (DEBUG)
                ForgeRegistryLog.log.trace("Registry {} remove: {} {}", this.superType.getSimpleName(), key, id);
        }
...
private static class OverrideOwner
    {
        final String owner;
        final NativeLocation key;
        private OverrideOwner(String owner, NativeLocation key)
        {
            this.owner = owner;
            this.key = key;
        }
