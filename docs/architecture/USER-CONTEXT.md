...
Status: Physical Context V1 is implemented for Core command dispatch. The
[user preferences and resolved locations](USER-PREFERENCES.md) add a stable
per-user configuration home and role-specific paths. Existing Setup V1 records
and profile APIs retain their respective authorities.
...
overrides. Fresh user configuration defaults to `~/.workbench/`; earlier OS
configuration records remain readable until imported. Core command dispatch
now resolves an external runtime-state root
...
Setup V1 retains its one reviewed workspace. User Workspaces V1 now adds a
separate named registry and optional default. The remaining program changes
are owner-specific:
...
| Workspace selection | Named targets and a default are available through user settings. Per-target profile and tool selections still need a separate reviewed contract; explicit command targets continue to win. |
